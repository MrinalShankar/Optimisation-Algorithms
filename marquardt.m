...
while (mod_del_F>epsilon)&(i<=max_iterations)
    if alpha>=10000
        new_X=([x01;x02]-(eye(size(subs(subs(J,x1,x01),x2,x02)))/alpha)*(subs(subs(del_F,x1,x01),x2,x02)));
    elseif alpha<=0.0001
        new_X=([x01;x02]-inv(subs(subs(J,x1,x01),x2,x02))*(subs(subs(del_F,x1,x01),x2,x02)));
    else
        new_X=([x01;x02]-inv(subs(subs(J,x1,x01),x2,x02)+alpha*eye(size(subs(subs(J,x1,x01),x2,x02))))*(subs(subs(del_F,x1,x01),x2,x02)));
    end
